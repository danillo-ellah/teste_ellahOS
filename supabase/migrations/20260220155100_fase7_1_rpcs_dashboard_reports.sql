...
-- RPCs de Dashboard e Relatorios (Fase 7.1 Part 2)
...
BEGIN
  SELECT json_build_object(
    ...
    'pending_approvals', (
      ...
      WHERE tenant_id = p_tenant_id AND deleted_at IS NULL AND status = 'pending'
    ),
    'overdue_deliverables', (
      SELECT count(*) FROM job_deliverables jd JOIN jobs j ON j.id = jd.job_id
      WHERE j.tenant_id = p_tenant_id AND jd.deleted_at IS NULL AND j.deleted_at IS NULL
...
BEGIN
  RETURN (
    SELECT json_agg(row_to_json(t)) FROM (
      ...
      FROM jobs WHERE tenant_id = p_tenant_id AND deleted_at IS NULL AND status NOT IN ('cancelado')
      GROUP BY status
...
-- 3. get_revenue_by_month
CREATE OR REPLACE FUNCTION get_revenue_by_month(p_tenant_id UUID, p_months INT DEFAULT 12)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN (
    SELECT json_agg(row_to_json(t)) FROM (
      ...
      FROM jobs j
      WHERE j.tenant_id = p_tenant_id AND j.deleted_at IS NULL AND j.status NOT IN ('cancelado')
...
-- 4. get_alerts
CREATE OR REPLACE FUNCTION get_alerts(p_tenant_id UUID, p_limit INT DEFAULT 20)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN (
    SELECT json_agg(row_to_json(t)) FROM (
      ...
      SELECT * FROM (
        ...
        WHERE ar.tenant_id = p_tenant_id AND ar.deleted_at IS NULL
...
CREATE OR REPLACE FUNCTION get_recent_activity(p_tenant_id UUID, p_hours INT DEFAULT 48, p_limit INT DEFAULT 30)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN (
    SELECT json_agg(row_to_json(t)) FROM (
