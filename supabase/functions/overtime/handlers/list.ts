import type { AuthContext } from '../../_shared/auth.ts';
import { AppError } from '../../_shared/errors.ts';
import { success } from '../../_shared/response.ts';
import { getSupabaseClient } from '../../_shared/supabase-client.ts';

export async function handleList(req: Request, auth: AuthContext): Promise<Response> {
  const url = new URL(req.url);
  const jobId = url.searchParams.get('job_id');

  if (!jobId) {
    throw new AppError('VALIDATION_ERROR', 'job_id e obrigatorio', 400);
  }

  console.log('[overtime/list] listando lancamentos', {
    userId: auth.userId,
    tenantId: auth.tenantId,
    jobId,
  });

  const client = getSupabaseClient(auth.token);

  // Verificar que o job pertence ao tenant
  const { data: job } = await client
    .from('jobs')
    .select('id')
    .eq('id', jobId)
    .eq('tenant_id', auth.tenantId)
    .maybeSingle();

  if (!job) {
    throw new AppError('NOT_FOUND', 'Job nao encontrado', 404);
  }

  const { data: entries, error: fetchError } = await client
    .from('time_entries')
    .select(`
      *,
      job_team!team_member_id (
        id,
        role,
        person_id,
        people!person_id (
          id,
          name
        )
      ),
      approver:profiles!approved_by (
        id,
        full_name
      )
    `)
    .eq('job_id', jobId)
    .eq('tenant_id', auth.tenantId)
    .is('deleted_at', null)
    .order('entry_date', { ascending: false })
    .order('team_member_id', { ascending: true });

  if (fetchError) {
    console.error('[overtime/list] erro ao buscar:', fetchError.message);
    throw new AppError('INTERNAL_ERROR', 'Erro ao buscar lancamentos de horas extras', 500, {
      detail: fetchError.message,
    });
  }

  console.log('[overtime/list] retornando', entries?.length ?? 0, 'lancamentos');
  return success(entries ?? [], 200, req);
}
